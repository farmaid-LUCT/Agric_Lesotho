...
class RuleMatchingService:
    ...
    def get_best_match(cls, disease_name, farmer, crop_profile,
        ...
        # ── BUG FIX: treat blank string ("") the same as NULL for all
        # optional trigger fields. Rules saved via admin with an empty
        # field store "" not NULL, which caused __isnull=True to miss them.
        # ─────────────────────────────────────────────────────────────────
        matching_rules = PersonalizedRule.objects.filter(
            Q(DiseaseName__iexact=disease_name)

            # District: match explicit value OR null/blank (= any district)
            & (
                Q(TriggerDistrict__iexact=gps_district)
                | Q(TriggerDistrict__isnull=True)
                | Q(TriggerDistrict='')
            )

            # Altitude tier: match explicit value OR 'any'
            & (
                Q(TriggerAltitudeTier=altitude_tier)
                | Q(TriggerAltitudeTier='any')
            )

            # Soil type: match explicit value OR null/blank (= any soil)
            & (
                Q(TriggerSoilType=crop_profile.SoilEnvironment)
                | Q(TriggerSoilType__isnull=True)
                | Q(TriggerSoilType='')
            )

            # Irrigation: match explicit value OR null/blank (= any irrigation)
            & (
                Q(TriggerIrrigation=crop_profile.irrigation_method)
                | Q(TriggerIrrigation__isnull=True)
                | Q(TriggerIrrigation='')
            )

            # Days since planting window
            & Q(MinDaysSincePlanting__lte=days_planted)
            & Q(MaxDaysSincePlanting__gte=days_planted)

            # Crop variety: match explicit value OR null/blank (= any variety)
            & (
                Q(TriggerCropVariety__iexact=crop_profile.seed_variety)
                | Q(TriggerCropVariety__isnull=True)
                | Q(TriggerCropVariety='')
            )

            # Season: match explicit value OR 'any'
            & (
                Q(TriggerSeason=current_season)
                | Q(TriggerSeason='any')
            )

            # Rainfall level: match explicit value OR 'any'
            & (
                Q(TriggerRainfallLevel=rainfall_level)
                | Q(TriggerRainfallLevel='any')
            )
        ).order_by('-priority_score')
