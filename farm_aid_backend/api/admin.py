...
class MonitorOnlyAdmin(admin.ModelAdmin):
    """Prevents Admin from adding, changing, or deleting records."""
...
class TranslationCacheAdmin(admin.ModelAdmin):
    """Admin manually manages Sesotho translations here."""
...
class PlantAdmin(MonitorOnlyAdmin):
    """Automatically populated via Flutter/Supabase."""
...
class WeatherDataAdmin(MonitorOnlyAdmin):
    """Automatically updated via Weather API."""
...
class AppAlertAdmin(MonitorOnlyAdmin):
    """Automatically generated by system logic."""
...
@admin.register(AIModel)
class AIModelAdmin(MonitorOnlyAdmin):
    list_display = ('Version', 'accuracy_rate_bar', 'LastTrainedDate')
    def accuracy_rate_bar(self, obj):
        return format_html('<div style="width:100px;background:#f1f1f1;border-radius:5px;display:inline-block;"><div style="width:{}%;background:#2e7d32;height:10px;border-radius:5px;"></div></div> {}%', obj.AccuracyRate, obj.AccuracyRate)
