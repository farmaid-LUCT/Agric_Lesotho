...
#         })
#     except Exception as e:
#         return Response({'error': str(e)}, status=500) rreeeeewedgcxvc
...
# ── 15. ADMIN DASHBOARD STATS VIEW (NEW) ──────────────────────────────────────
...
class AdminDashboardStatsView(APIView):
    ...
    def get(self, request):
        ...
        # 1. Total scans (from Plant model)
        total_scans = Plant.objects.count()
        
        # 2. Diseases detected (unique diseases from Diagnosis)
        diseases_detected = Diagnosis.objects.values('DiseaseName').distinct().count()
        
        # 3. Identification accuracy (average confidence)
        avg_accuracy = Diagnosis.objects.aggregate(
            avg=Avg('ConfidenceLevel')
        )['avg'] or 0.0
        
        # 4. New diseases logged this month
        this_month = now.month
        new_diseases = Diagnosis.objects.filter(
            DateDiagnosed__month=this_month
        ).values('DiseaseName').distinct().count()
        
        # 5. Weekly scans (last 7 days)
        weekly_scans = {}
        for i in range(6, -1, -1):
            day = now - timedelta(days=i)
            day_name = day.strftime('%A')
            count = Plant.objects.filter(
                DateCaptured__date=day.date()
            ).count()
            weekly_scans[day_name] = count
        
        # 6. Unverified reports (confidence < 85% and no feedback)
        unverified_diagnoses = Diagnosis.objects.filter(
            ConfidenceLevel__lt=0.85,
            farmer_feedback__isnull=True
        ).select_related('PlantID__FarmerID')[:5]
        
        unverified_reports = []
        for d in unverified_diagnoses:
            unverified_reports.append({
                'id': d.DiagnosisID,
                'plant_image': d.PlantID.ImageFile if d.PlantID else '',
                'pred_disease': d.DiseaseName,
                'confidence': round(d.ConfidenceLevel * 100, 1),
                'location': d.PlantID.gps_district or 'Unknown'
            })
        
        # 7. Disease distribution (percentage of each disease)
        disease_counts = Diagnosis.objects.values('DiseaseName').annotate(
            count=Count('DiseaseName')
        )
        total_diagnoses = sum(dc['count'] for dc in disease_counts)
        disease_distribution = {}
        for dc in disease_counts:
            if total_diagnoses > 0:
                disease_distribution[dc['DiseaseName']] = round(
                    (dc['count'] / total_diagnoses) * 100, 1
                )
        ...
        if not disease_distribution:
            disease_distribution = {
                ...
                'Powdery Mildew': 25.0,
                'Mosaic Virus': 15.0,
                'Aphid Infestation': 12.0,
                'Others': 18.0,
            }
        
        # 8. System health (mock data - can be enhanced later)
        system_health = {
            'Disease Prediction Model': 99.7,
            'Farm Data Lake': 'Optimal',
            'Diagnostic Image Store': 78,
        }
        
        # 9. Recent activities (last 5 activities)
        recent_activities = []
        
        # Get recent diagnoses
        recent_diagnoses = Diagnosis.objects.select_related('PlantID__FarmerID').order_by('-DateDiagnosed')[:3]
        for d in recent_diagnoses:
            farmer_name = d.PlantID.FarmerID.username if d.PlantID and d.PlantID.FarmerID else 'Unknown'
            recent_activities.append({
                'message': f"Farmer {farmer_name} reported {d.DiseaseName.replace('_', ' ')}",
                'timestamp': d.DateDiagnosed.isoformat()
            })
        
        # Add some sample activities if needed
        if len(recent_activities) < 3:
            recent_activities = [
                {'message': 'Farmer John D. reported Early Blight on Tomatoes', 'timestamp': now.isoformat()},
                {'message': 'Field Team #345 updated Aphid model data', 'timestamp': now.isoformat()},
                {'message': 'AI flagged a new type of Mosaic on Cucumbers', 'timestamp': now.isoformat()},
            ]
        
        return Response({
            'total_scans': total_scans,
            'diseases_detected': diseases_detected,
            'identification_accuracy': round(avg_accuracy * 100, 1),
            'new_diseases_logged': new_diseases,
