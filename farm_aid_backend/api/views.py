...
#             image_url = request.data.get('imageUrl') or request.data.get('image_url') or request.data.get('ImageFile') or ''
#             confidence = request.data.get('confidence') or request.data.get('ConfidenceLevel') or 0.0
#             profile_id = request.data.get('profileId') or request.data.get('ProfileID')
...
class SaveScanView(APIView):
    ...
    def _get_sesotho(self, disease_name, field='pesticide'):
        ...
        TranslationCache model fields:
          disease_name_en  (PK)
          pesticide_st, dosage_st, steps_st

        Looks up by disease name, returns the requested Sesotho field.
        Returns None if no row exists — caller keeps English silently.
    ...
    def post(self, request):
        ...
        try:
            ...
            # 4. Sesotho translation — keyed by disease name, not raw text
            # TranslationCache: disease_name_en PK → pesticide_st / dosage_st / steps_st
            # Falls back to English silently when admin hasn't entered translation yet
            if lang == 'st':
