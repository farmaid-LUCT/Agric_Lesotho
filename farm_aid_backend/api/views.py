...
class SaveScanView(APIView):
    ...
    def _get_manual_sesotho(self, disease_name, field='pesticide'):
        """
        Looks up Sesotho translation from TranslationCache using disease_name_en as key.
        TranslationCache model fields:
          disease_name_en (PK), pesticide_st, dosage_st, steps_st
        Falls back to English if no translation exists.
        field: 'pesticide' | 'dosage' | 'steps'
        """
        if not disease_name:
            return None
        try:
            ...
            return field_map.get(field) or None
    ...
    def post(self, request):
        try:
            ...
            crop_type = (target_profile.VegetableType
                         ...
                         request.data.get('cropType', 'Vegetable'))

            # ── 3. Save Plant (with GPS) ───────────────────────────────────
            new_plant = Plant.objects.create(
                FarmerID    = user,
                CropProfile = target_profile,
                CropType    = crop_type,
            ...
            # ── 4. Save Diagnosis (keep reference for id + follow_up_date) ─
            from datetime import timedelta
            is_serious = any(w in clean_label.lower()
                             for w in ['blight', 'rot', 'wilt', 'mold', 'virus',
                                       'bacteria', 'phytophthora', 'fusarium'])
            follow_up_date = date.today() + timedelta(days=3 if is_serious else 10)
            ...
            # TranslationCache is keyed by disease_name_en (PK).
            # Fields: pesticide_st, dosage_st, steps_st.
            # We look up by the DISEASE NAME, not the text content.
            if lang == 'st':
                st_pesticide = self._get_manual_sesotho(clean_label, 'pesticide')
                st_dosage    = self._get_manual_sesotho(clean_label, 'dosage')
                st_steps     = self._get_manual_sesotho(clean_label, 'steps')
                # Only replace if a Sesotho translation actually exists
                if st_pesticide: res_pesticide = st_pesticide
                if st_dosage:    res_dosage    = st_dosage
                if st_steps:     res_steps     = st_steps

            # ── 7. Personalized advice (8-factor engine) ──────────────────
            personalized_advice = None
            if target_profile:
                try:
                    from .models import RuleMatchingService, WeatherData
                    ...
                    pass   # engine failure must never break the scan response
