...
# ... (Auth views remain the same as previously provided) ...

class SaveScanView(APIView):
    permission_classes = [IsAuthenticated]

    def _get_manual_sesotho(self, english_disease_name, field_type):
        """
        Pulling directly from TranslationCache if Sesotho is selected.
        Fields: disease_name_en, pesticide_st, dosage_st, steps_st
        """
        # Search translation table by English disease name
        cached = TranslationCache.objects.filter(disease_name_en__iexact=english_disease_name).first()
        
        if cached:
            if field_type == "pesticide": return cached.pesticide_st or "Iteanye le setsebi"
            if field_type == "dosage": return cached.dosage_st or "N/A"
            if field_type == "steps": return cached.steps_st or "Arola semela hang-hang."
            if field_type == "disease": return getattr(cached, 'disease_name_st', english_disease_name) or english_disease_name
    ...
    def post(self, request):
        try:
            with transaction.atomic():
                ...
                # 1. Setup Language and Labels
                lang = data.get('language') or user.language_preferences or 'en'
                raw_label = data.get('diseaseName', 'Healthy')
                clean_label = raw_label.split('___')[-1].replace('_', ' ').strip()
                
                # 2. Save Basic Record (Always happens)
                new_plant = Plant.objects.create(
                    FarmerID=user, 
                    ImageFile=data.get('imageUrl', ""),
                    CropType="Vegetable"
                )
                Diagnosis.objects.create(
                    PlantID=new_plant, 
                    DiseaseName=clean_label, 
                    ConfidenceLevel=float(data.get('confidence', 0.0))
                )

                # 3. CONDITIONAL LOGIC BASED ON LANGUAGE
                if lang == 'en':
                    # WORK NORMAL: Pull from Treatment/KnowledgeBase
                    treat = Treatment.objects.filter(DiseaseName__icontains=clean_label).first()
                    kb = KnowledgeBase.objects.filter(DiseaseName__icontains=clean_label).first()

                    res_disease = clean_label
                    res_pesticide = treat.RecommendedPesticide if treat else "Consult local expert"
                    res_dosage = treat.Dosage if treat else "N/A"
                    res_steps = treat.ApplicationSteps if treat else (kb.TreatmentInfo if kb else "Isolate plant immediately.")
                
                else:
                    # SESOTHO: Pull strictly from Translation Table
                    res_disease = self._get_manual_sesotho(clean_label, "disease")
                    res_pesticide = self._get_manual_sesotho(clean_label, "pesticide")
                    res_dosage = self._get_manual_sesotho(clean_label, "dosage")
                    res_steps = self._get_manual_sesotho(clean_label, "steps")
...
# ... (Rest of views: FarmerHistoryView, FarmerReportsView, etc.) ...
