        ...
        for p in plants:
            ...
            if diag:
                ...
                report_data.append({
                    ...
                    'farmer_feedback': diag.farmer_feedback,
                    'causes': causes,
                    'personalized_advice': personalized_advice,  # ← MAKE SURE THIS LINE EXISTS
...
class FarmerInsightView(APIView):
    ...
    def get(self, request):
        ...
        insight.total_diseases_detected = total_diseases
...
#             # ============================================================
#             # APPLY SPICY MARIANMT TRANSLATIONS IF LANGUAGE IS 'st'
#             # ============================================================
#             if lang == 'st':
#                 logger.warning(f"[SaveScan] 🔥 Applying Spicy MarianMT Sesotho translation for disease: '{clean_label}'")
                
#                 from .translation_service import translate_treatment_batch
                
#                 # Translate all three treatment fields using MarianMT (with caching & learning)
#                 translations = translate_treatment_batch(res_pesticide, res_dosage, res_steps)
                
#                 res_pesticide = translations['pesticide_st']
#                 res_dosage = translations['dosage_st']
#                 res_steps = translations['steps_st']
                
#                 logger.warning("[SaveScan] ✅ Spicy MarianMT translations applied and stored in TranslationCache")

#             # ============================================================
#             # PERSONALIZED MODE - ALWAYS GENERATE IN ENGLISH, THEN TRANSLATE
#             # ============================================================
#             personalized_advice = None
#             matched_context = None
#             personalized_dosage = None
...
#             if wants_personalized and target_profile:
#                 logger.warning("[SaveScan] 📝 Generating personalized advice in English...")
                
#                 # Always generate in English first (single source of truth)
#                 personalized = self._generate_personalized_advice(
#                     disease_name=clean_label,
#                     farmer=user,
#                     crop_profile=target_profile,
...
#                 personalized_advice_en = personalized['advice']
#                 matched_context = personalized['matched_on']
                
#                 # If user wants Sesotho, translate with MarianMT
#                 if lang == 'st':
#                     logger.warning("[SaveScan] 🔥 Translating personalized advice to Sesotho with MarianMT...")
#                     from .translation_service import translate_text
#                     personalized_advice = translate_text(personalized_advice_en, field_type='general', store_for_learning=True)
#                     logger.warning("[SaveScan] ✅ Personalized advice translated to Sesotho")
#                 else:
#                     personalized_advice = personalized_advice_en
...
# # ── 15. SPICY MARIANMT API ENDPOINTS ─────────────────────────────────────────

# @api_view(['POST'])
# @permission_classes([IsAuthenticated])
# def spicy_translate(request):
#     """Spicy translation endpoint - caches everything for continuous learning"""
#     from .translation_service import translate_text
...
#     field_type = request.data.get('field_type', 'general')
#     store = request.data.get('store_for_learning', True)
    
#     translated = translate_text(text, field_type=field_type, store_for_learning=store)
    
#     return Response({
#         'original': text,
#         'translated': translated,
#         'model': 'Helsinki-NLP/opus-mt-en-ss',
...
# @api_view(['POST'])
# @permission_classes([IsAuthenticated])
# def spicy_batch_translate(request):
#     """Batch translation for multiple texts - spicy parallel processing"""
...
#     field_type = request.data.get('field_type', 'general')
#     translations = translate_batch(texts, field_type)
...
# @api_view(['GET'])
# @permission_classes([IsAuthenticated])
# def spicy_cache_stats(request):
#     """Get spicy cache statistics - monitor your learning progress"""
#     from .translation_service import get_cache_stats
...
# @api_view(['GET'])
# @permission_classes([IsAuthenticated])
# def spicy_health(request):
#     """Health check for spicy translation service"""
#     from .translation_service import _get_model
    
#     try:
#         _get_model()
#         return Response({
#             'status': '🔥 SPICY AND HEALTHY',
#             'model': 'Helsinki-NLP/opus-mt-en-ss',
#             'ready': True,
#         })
#     except Exception as e:
