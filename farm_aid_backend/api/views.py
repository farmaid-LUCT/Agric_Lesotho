...
# ena le entse ke sebetsana le yona now
...
class SaveScanView(APIView):
    ...
    def _get_sesotho(self, disease_name, field='pesticide'):
        """
        Looks up Sesotho from TranslationCache.
        Model: disease_name_en (PK), pesticide_st, dosage_st, steps_st.
        Returns None when no entry exists — caller keeps English.
        """
        if not disease_name:
            return None
        try:
            cache = TranslationCache.objects.get(disease_name_en__iexact=disease_name)
            return {'pesticide': cache.pesticide_st,
    ...
    def post(self, request):
        ...
        try:
            ...
            # 4. Sesotho translation — keyed by disease name
